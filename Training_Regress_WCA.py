...
#directory = Path('C:\\Users\\josea\\MEGA\\NESCGLE_scripts\\DateBase\\DataBases_WCA_visc\\DataBase_WCA_vis2')
...
train_set, test_set = train_test_split(df_copy, test_size=0.6, random_state=42)
...
X_train_Tnp = df_T_train.to_numpy()
X_val_Tnp = df_T_val.to_numpy()
X_test_Tnp = df_T_test.to_numpy()
...
X_train_Tor = pd.Series(X_train_Tnp, name='T')
X_val_Tor = pd.Series(X_val_Tnp, name='T')
X_test_Tor = pd.Series(X_test_Tnp, name='T')

df_train = df_separater2(columnas, RX_train_sn, X_train_phior, X_train_Tor)
#df_train = df_separater(columnas, RX_train_sn)

df_val = df_separater2(columnas, RX_val_sn, X_val_phior, X_val_Tor)
#df_val = df_separater(columnas, RX_val_sn)

df_test = df_separater2(columnas, RX_test_sn, X_test_phior, X_test_Tor)
...
#ump(lasso, 'log_viscosidadesSW_las.joblib')
...
train_score_r = ridge.score(df_train, Y_train_or)
print(train_score_r)
val_score_r = ridge.score(df_val, Y_val_or)
print(val_score_r)
test_score_r = ridge.score(df_test, Y_test_or)
print(test_score_r)
#ump(ridge, 'log_viscosidadesSWr.joblib')